...
class MISADataset(BaseDataset):
    @staticmethod
    def modify_commandline_options(parser, isTrain=None):
        parser.add_argument('--cvNo', type=int, help='which cross validation set')  # 交叉验证集标识
    ...
    def __init__(self, opt, set_name):
        ...
        pwd = os.path.abspath(__file__)     # 返回脚本的绝对路径
        pwd = os.path.dirname(pwd)          # 返回脚本的父路径
        config = json.load(open(os.path.join(pwd, 'config', f'{opt.corpus_name}_config.json'))) # 从配置文件加载配置
        self.norm_method = opt.norm_method  # 如何标准化特征
        self.corpus_name = opt.corpus_name  # 选择何种数据集
        # load feature
...
if __name__ == '__main__':
    ...
    a = MISADataset(opt, set_name='trn')
    data = next(iter(a))
