...
if __name__ == '__main__':
    ...
    if opt.has_test:
        ...
        #     TSNE_model = TSNE()
        #
        #     # utt_shared_t = TSNE_model.fit_transform(model.utt_shared_t.cpu().detach().numpy())
        #     # utt_shared_v = TSNE_model.fit_transform(model.utt_shared_v.cpu().detach().numpy())
        #     # utt_shared_a = TSNE_model.fit_transform(model.utt_shared_a.cpu().detach().numpy())
        #
        #     utt_shared_miss = TSNE_model.fit_transform(model.shared_miss.cpu().detach().numpy())
        #     utt_shared = TSNE_model.fit_transform(model.shared.cpu().detach().numpy())
        #
        #     # plt.scatter(x=utt_shared_t[:, 0], y=utt_shared_t[:, 1], c='b', marker='o', label='utt_shared_t')
        #     # plt.scatter(x=utt_shared_v[:, 0], y=utt_shared_v[:, 1], c='r', marker='o', label='utt_shared_v')
        #     # plt.scatter(x=utt_shared_a[:, 0], y=utt_shared_a[:, 1], c='k', marker='o', label='utt_shared_a')
        #     plt.scatter(x=utt_shared_miss[:, 0], y=utt_shared_miss[:, 1], c='b', marker='x', label='utt_shared_miss')
        #     plt.scatter(x=utt_shared[:, 0], y=utt_shared[:, 1], c='r', marker='o', label='utt_shared')
        #     plt.legend(loc='best')
        #     plt.figure(num=1, figsize=(256, 256))
        #     # plt.show()  #这句是显示检测好的图片。如果加这句，下面的保存图片时，会是空图，不回，保存的图片是正确的，速度也快
        #     plt.savefig(str(opt.cvNo) + '-' + str(best_eval_epoch) + '-' + 'MISA_shared.jpg')
    else:
        result_recorder.write_result_to_tsv({
            'acc': best_eval_acc,
            'uar': best_eval_uar,
