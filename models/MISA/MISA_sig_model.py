...
class MISASIGModel(BaseModel):
    ...
    def __init__(self, config):
        ...
        config.hidden_size = config.embd_size_v + config.embd_size_l + config.embd_size_a
        ...
        # 以下是MMIN配置文件的内容
        self.model_names = ['L1', 'L2', 'V1', 'V2', 'A1', 'A2', 'project_t', 'project_v', 'project_a',
                            'private_t', 'private_v', 'private_a', 'shared', 'recon_t', 'recon_v', 'recon_a',
        ...
        # self.netL1 = TextCNN(input_sizes[0], hidden_sizes[0])
        # self.netL2 = TextCNN(2 * hidden_sizes[0], hidden_sizes[0])
        ...
        # self.netV1 = LSTMEncoder(input_sizes[1], hidden_sizes[1], embd_method=self.config.embd_method_v)
        # self.netV2 = LSTMEncoder(2 * hidden_sizes[1], hidden_sizes[1], embd_method=self.config.embd_method_v)
        ...
        # self.netA1 = LSTMEncoder(input_sizes[2], hidden_sizes[2], embd_method=self.config.embd_method_a)
        # self.netA2 = LSTMEncoder(2 * hidden_sizes[2], hidden_sizes[2], embd_method=self.config.embd_method_a)
        ...
        self.netshared.add_module('shared_1', nn.Linear(in_features=config.hidden_size, out_features=config.hidden_size))
        self.netshared.add_module('shared_1_activation', nn.LeakyReLU())
        self.netshared.add_module('shared_1_dropout', nn.Dropout(dropout_rate))
        self.netshared.add_module('shared_2', nn.Linear(in_features=config.hidden_size, out_features=config.hidden_size))
        self.netshared.add_module('shared_2_activation', nn.LeakyReLU())
        self.netshared.add_module('shared_2_dropout', nn.Dropout(dropout_rate))
        self.netshared.add_module('shared_3', nn.Linear(in_features=config.hidden_size, out_features=config.hidden_size))
        self.netshared.add_module('shared_3_activation', nn.LeakyReLU())
        self.netshared.add_module('shared_3_dropout', nn.Dropout(dropout_rate))
        self.netshared.add_module('shared_last', nn.Linear(in_features=config.hidden_size, out_features=config.hidden_size))
        self.netshared.add_module('shared_activation_1', nn.Sigmoid())
        ...
        self.netsp_discriminator.add_module(
            ...
            nn.Linear(in_features=config.hidden_size, out_features=256))
        self.netsp_discriminator.add_module(
        ...
        # 使用textcnn
        # self.nettlayer_norm = nn.LayerNorm((hidden_sizes[0]))
        # self.netvlayer_norm = nn.LayerNorm((hidden_sizes[1]))
        # self.netalayer_norm = nn.LayerNorm((hidden_sizes[2]))
    ...
    def forward(self):
        ...
        final_h1v, final_h2v = self.extract_features(self.visual, 'lstm', self.netV1, self.netV2,
                                                     self.netvlayer_norm)
        # print('final_hv.size is:', final_h1v.size())
    ...
    def get_cmd_loss(self, ):
        ...
        # loss = mse(self.utt_shared_t, self.utt_shared_v)
        # loss += mse(self.utt_shared_t, self.utt_shared_a)
        # loss += mse(self.utt_shared_a, self.utt_shared_v)
        # loss = loss / 3.0
    ...
    def get_diff_loss(self):
        ...
        # between private and shared
        # mse = nn.MSELoss(reduction='mean')
        # loss = mse(private_v, shared_v)
        # loss += mse(private_a, shared_a)
        # loss += mse(private_t, shared_t)
        #
        # # across privates
